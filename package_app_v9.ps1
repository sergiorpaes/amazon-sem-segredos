...
$tempDir = Join-Path $projectPath "temp_dist_source_final_v9"
...
$robocopyParams = @(
    ...
    $tempDir,
    "/E",
    "/XD", "node_modules", ".git", "dist", ".netlify", ".gemini", "temp_dist_source_final*", "pages", "pages_temp", "Pages", "views" 
)
...
# Explicit Check for src/index.tsx
if (-not (Test-Path "$tempDir\src\index.tsx")) {
    Write-Error "CRITICAL: 'src\index.tsx' missing!"
}
