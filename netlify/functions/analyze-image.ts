...
export const handler = async (event: any) => {
    ...
    try {
        ...
        async function tryGenerate(modelName: string) {
            ...
            - features: Product detail with text overlays in ${language === 'pt' ? 'Português' : language === 'es' ? 'Espanhol' : 'Inglês'} pointing to features (ex: "Lâmina de Aço Inoxidável", "Trava de Segurança").
            - exploded: Parts separated clearly with labels in ${language === 'pt' ? 'Português' : language === 'es' ? 'Espanhol' : 'Inglês'} for each component.
            ` : `
            Gere 3 prompts de estilo Lifestyle:
            1. "lifestyle" shot (e.g., in use, home setting).
            2. "creative" shot (e.g., studio lighting, artistic background).
            3. "application" shot (e.g., showing the benefit/result).
            `}
            ...
            Return the result as a STRICT JSON object with these keys:
            {
              ...
              "prompts": {
                    ...
                    "creative": "Full prompt for studio/creative shot...",
                    "application": "Full prompt showing application/benefit..."
                    ${isPro ? `,
